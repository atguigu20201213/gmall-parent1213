...
//       BaseTrademark trademark= productFeignClient.getTrademarkByTmId(skuInfo.getTmId());
    ...
    public SearchResponseVo search(SearchParam searchParam) throws Exception {
        ...
        //根据总条数显示的多少来计算  全新的 公式计算总页数
       // long totalPages=responseVo.getTotalPages()+searchParam.getPageSize()-1/searchParam.getPageSize();
...
//                ((Terms.Bucket) bucket).getKeyAsString()
                //声明一个对象
                ...
                //获取集合中的每个数据
                //Terms.Bucket::getKeyAsString   通过key 获取value
...
//        返回对象
    ...
    private SearchRequest buildQueryDsl(SearchParam searchParam) {
        ...
        //查询品牌  判断用户是否输入； 判断条件 查询参数   ： {trademark :  华为}
        //获取用户查询的品牌数据
        String trademark = searchParam.getTrademark();
        if (!StringUtils.isEmpty(trademark)) {
            //用户输入了品牌查询 通过key 2;华为 将value 进行分割
        ...
        if (!StringUtils.isEmpty(order)) {
            ...
            if (null != split && split.length == 2) {
                ...
                //searchSourceBuilder.sort(filed, SortOrder.DESC);
        ...
        //数据结果集进行过滤    结果集显示  id"id","defaultImg","title","price"}
        searchSourceBuilder.fetchSource(new String[]{"id","defaultImg","title","price"},null);

        //指定 index   type  get /goods /info /_search{}
